...
public class AbilityHandler {
	...
	private static void tickBootsAbilities(EntityLivingBase entity) {
		...
		if (entity.onGround || flying || swimming) {
			...
			if (entity.distanceWalkedModified > 0f) {
				entity.move(MoverType.SELF, 0f, 1f, speed);
			}
			else if (entity.distanceWalkedModified < 0f) {
				entity.move(MoverType.SELF, 0f, 1f, -speed * 0.3f);
			}

			if (entity.moveStrafing != 0f) {
				entity.move(MoverType.SELF, 1f, 0f, speed * 0.5f * Math.signum(entity.moveStrafing));
			}
		}
	}
	//endregion
